...
#         print(f"dataset_config['features'] = {dataset_config['features']}")
...
#                 texts = df[field].fillna('').tolist()
#                 embeddings = self.text_model.encode(texts, show_progress_bar=False)
#                 df_processed[f'{field}_embedding'] = list(embeddings)
                
#         return df_processed
    
#     def _process_categorical_features(self, df: pd.DataFrame, categorical_fields: List[str], is_train: bool) -> pd.DataFrame:
#         df_processed = df.copy()
...
#                 df_processed[field] = df_processed[field].astype('int64')
        
#         return df_processed
    
#     def _process_numerical_features(self, df: pd.DataFrame, numerical_fields: List[str], is_train: bool) -> pd.DataFrame:
#         df_processed = df.copy()
...
#                 if is_train:
#                     self.scalers[field] = StandardScaler()
#                     df_processed[field] = self.scalers[field].fit_transform(df_processed[[field]].fillna(0))
#                 else:
#                     df_processed[field] = self.scalers[field].transform(df_processed[[field]].fillna(0))
                
#                 df_processed[field] = df_processed[field].astype('float32')
        
#         return df_processed
    
#     def process_features(self, df: pd.DataFrame, feature_config: Dict, is_train: bool = True) -> pd.DataFrame:
#         dataset_config = feature_config.get('features', {})
#         text_fields = dataset_config.get('text_features', [])
#         categorical_fields = dataset_config.get('categorical_features', [])
#         numerical_fields = dataset_config.get('numerical_features', [])
...
class FeaturePreprocessor:
    ...
    def _process_text_features(self, df: pd.DataFrame, text_fields: List[str], max_seq_length: int = 50, model_type: str = 'sasrec') -> pd.DataFrame:
        ...
        for field in text_fields:
            if field in df.columns:
                texts = df[field].fillna('').tolist()
                embeddings = torch.tensor(self.text_model.encode(texts, show_progress_bar=True), device=self.device)
                df_processed[f'{field}_embedding'] = embeddings.cpu().numpy().tolist()
                
                if model_type == 'bert4rec':
                    user_sequences = df.groupby('viewer_uid')[f'{field}_embedding'].apply(list).to_dict()
                    for user_id in user_sequences:
                        seq = user_sequences[user_id][:max_seq_length] + [[0.0] * embeddings.shape[1]] * (max_seq_length - len(user_sequences[user_id]))
                        user_sequences[user_id] = seq[:max_seq_length]
                    df_processed[f'{field}_embedding_list'] = df['viewer_uid'].map(user_sequences)
    ...
    def process_features(self, df: pd.DataFrame, feature_config: Dict, dataset_type: str, is_train: bool = True, model_type: str = 'sasrec') -> pd.DataFrame:
        dataset_config = feature_config.get(dataset_type, feature_config)
        text_fields = dataset_config['features'].get('text_features', [])
        categorical_fields = dataset_config['features'].get('categorical_features', [])
        numerical_fields = dataset_config['features'].get('numerical_features', [])
        max_seq_length = dataset_config.get('MAX_ITEM_LIST_LENGTH', 50)
        ...
        if text_fields:
            df = self._process_text_features(df, text_fields, max_seq_length, model_type)
        if categorical_fields:
            df = self._process_categorical_features(df, categorical_fields, is_train)
        if numerical_fields:
            df = self._process_numerical_features(df, numerical_fields, is_train)
...
def get_full_features_config(dataset_config: Dict) -> Dict:
    ...
    categorical_embeddings = [f'{field}_embedding' for field in categorical_features]
    
    return {
        'numerical_features': numerical_features + text_embeddings + categorical_embeddings,
        'categorical_features': categorical_features,
        # 'text_fields': text_fields,
        'embedding_size': dataset_config.get('embedding_size', self.embedding_dim),
        'numerical_projection_dropout': dataset_config.get('numerical_projection_dropout', 0.1)
    }